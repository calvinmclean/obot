...
func (p *PowerUserWorkspaceHandler) ListAllEntries(req api.Context) error {
	...
	for _, item := range list.Items {
		...
		for _, entry := range list2.Items {
			catalogEntries = append(catalogEntries, ConvertMCPServerCatalogEntryWithWorkspace(entry, item.Name, item.Spec.UserID, p.serverURL, nil))
		}
	}
...
func (p *PowerUserWorkspaceHandler) ListAllServers(req api.Context) error {
	...
	for _, server := range filteredServers {
		credCtxs = append(credCtxs, mcp.MCPServerCredentialContextForScope(server.Spec.PowerUserWorkspaceID, server.Name))
	}
...
func (p *PowerUserWorkspaceHandler) ListAllServersForAllEntries(req api.Context) error {
	...
	credCtxs := make([]string, 0, len(filteredServers))
	for _, server := range filteredServers {
		credCtxs = append(credCtxs, mcp.MCPServerCredentialContextForUser(server.Spec.UserID, server.Name))
	}
